...
g_mean_all <- normalized_features_long_pop %>%  ggplot() +
  ...
  geom_polygon(
    data = . %>%  group_by(artist_name, metric) %>%
      summarise_at(c("value"), mean) %>%
      arrange(factor(metric, levels = order)) %>%
      ungroup(),
    aes(
...
g_mean_ind <-  normalized_features_long_pop %>%  ggplot(aes(x=metric,y=value,group=interaction(artist_name,metric))) +
  scale_x_discrete(
    limits = order,
    labels = c(
      "آکوستیک",
  ...
  geom_polygon(
    data = normalized_features_long_pop %>%  group_by(artist_name, metric) %>%
      summarise_at(c("value"), mean) %>%
      arrange(factor(metric, levels = order)) %>%
      ungroup(),
    aes(
...
songs_audio_plus_pop_jitter <- songs_audio_plus_pop %>% 
  #filter(artist_name_farsi %in% artists) %>% 
  ...
  mutate(track_farsi_avail = if_else(!is.na(track_name)& !is.na(popularity) & nchar(track_name) < 20 & !explicit,track_name,'')) 
```



```{r fig.height=40, fig.width=20,dpi=2000}
songs_audio_plus_pop_jitter %>%
  ...
  geom_text_repel(
    aes(label = track_farsi_avail , x = artist_name , y = valence),
    family = 'B Mitra',
    color = 'gray99',
    size = 7,
    force = 0.6,
...
songs_audio_plus_pop_jitter %>%
  ...
  geom_text_repel(
    aes(label = track_farsi_avail , x = artist_name , y = energy),
    family = 'B Mitra',
    color = 'gray90',
    size = 6,
    force = 0.6,
...
songs_audio_plus_pop_jitter %>%
  ...
  geom_text_repel(
    aes(label = track_farsi_avail , x = artist_name , y = acousticness),
    family = 'B Mitra',
    color = 'gray90',
    size = 6,
    force = 0.6,
...
songs_audio_plus_pop_jitter %>%
  ...
  geom_text_repel(
    aes(label = track_farsi_avail , x = artist_name , y = danceability),
    family = 'B Mitra',
    color = 'gray90',
    size = 6,
    force = 0.6,
...
songs_audio_plus_pop_jitter %>%
  ...
  geom_text_repel(
    aes(label = track_farsi_avail , x = artist_name , y = loudness),
    family = 'B Mitra',
    color = 'gray90',
    size = 6,
    force = 0.6,
