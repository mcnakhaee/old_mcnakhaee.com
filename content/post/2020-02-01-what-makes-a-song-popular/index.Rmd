...
theme_update(legend.position = 'top',
      ...
      plot.title = element_text(family = 'Montserrat', face = "bold", size = 50,hjust = 0.5, color = '#FFE66D'),
...
```{r eval=TRUE, fig.height=30, fig.width=30, warning=FALSE, include=TRUE}
embeddings %>%
  ...
  guides(size = FALSE,
    ...
    geom_text_repel(aes(label = popular_tracks_selected_artist),size = 6, family = 'Montserrat',
    point.padding = 2.2,
    box.padding = .5,
    force = 1,
...
```{r eval=FALSE, fig.height=35, fig.width=35, include=FALSE}
supervised_umap_embedding_ds %>%
  ggplot(aes(x = umap_1, y = umap_2 ,color = selected_artist )) +
  geom_point(aes(size = point_size_selected_artist)) +
  gghighlight(selected_artist != "",unhighlighted_params = list(alpha = 0.2, color = '#FFE66D')) +
