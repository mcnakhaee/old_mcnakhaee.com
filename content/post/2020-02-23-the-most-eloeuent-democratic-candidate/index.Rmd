---
title: "Analyzing Democratic Party's Debate - Part 1: The Most Eloeuent Democratic Candidate"
...
I use the Tidytext library and its stopword list to compute my defined metrics. But first the debate transcripts should be truned into a tidy format (one word per row). Then, I create a logical variable to see whether a words is a stopword or not.
...
```{r, fig.height = 7, fig.width = 14, message=FALSE, warning=FALSE}
 debate_vocab_df %>%
  ...
  geom_col(show.legend = FALSE) +
  geom_label(aes(label = round(vocab_size,digits = 3) ,size = 7)) +
  coord_flip() +
  scale_fill_manual(values = custom_palette) +
