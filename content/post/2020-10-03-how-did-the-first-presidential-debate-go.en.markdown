---
title: How Did the First Presidential Debate Go?
author: mcnakhaee
date: '2020-10-03'
slug: how-did-the-first-presidential-debate-go
categories:
  - R
  - NLP
tags:
...
summary: ''
authors: []
lastmod: '2020-10-03T05:39:56+02:00'
featured: no
draft: true
image:
  caption: ''
...
```r
debate_2020 <- read_delim('first_debate.csv',
                        delim = '\t',
...
debate_2016 <- debate_2016 %>% 
  ...
  filter(!Speaker %in% c('Kaine','Pence'),
    ...
    Speaker = case_when(Speaker == 'CANDIDATES'~'Crosstalk',
                        ...
                        Speaker %in% c('Cooper', 'Holt','Wallace','Raddatz') ~'Moderator',
                        TRUE ~ Speaker))


first_debate <- debate_2016 %>% 
  filter(Date == '9/26/16')
second_debate <- debate_2016 %>% 
  filter(Date == '10/9/16')
third_debate <- debate_2016 %>% 
  filter(Date == '10/19/2016')


first_debate<- first_debate %>% 
  mutate(cumsum_nchar = cumsum(num_chars),
         cum_sum_lag = lag(cumsum_nchar),
         debate = 'First Debate')
...
all_debates_2016 <- bind_rows(first_debate,second_debate ,third_debate)
```
...
all_debates_2016 %>%
  ...
  theme_void() +
  theme(
    legend.box.margin = margin(b = 40),
    strip.text = element_text(size = 25,family = 'Lobster'),
    legend.position = 'top',
    legend.text = element_text(
      size = 20,
      family = 'Lobster',
...
```r
gofastr::presidential_debates_2012
```
